...
export const songs = pgTable(
  ...
  (t) => [
    ...
    index('songs_slug_idx').on(t.slug),
...
export const certifications = pgTable(
  'certifications',
  {
    ...
    territory: text('territory').notNull(), // 'US' | 'UK' | 'NG' | 'GLOBAL'
    body: text('body').notNull(), // 'RIAA' | 'BPI' | 'IFPI'
    level: text('level').notNull(), // 'gold' | 'platinum' | 'diamond'
    units: bigint('units', { mode: 'number' }), // threshold units at certification
...
export const awardRecords = pgTable(
  'award_records',
  {
    ...
    awardName: text('award_name').notNull(), // 'Grammy' | 'BET' | 'MOBO' | 'MTV VMA'
    category: text('category').notNull(),
    result: text('result').notNull(), // 'won' | 'nominated'
    year: integer('year').notNull(),
    ceremony: text('ceremony'), // '67th Grammy Awards'
  },
  (t) => [
    index('award_records_artist_idx').on(t.artistId),
    index('award_records_award_year_idx').on(t.awardName, t.year),
    index('award_records_result_idx').on(t.awardName, t.result),
  ],
);
